...
class InternVideo2VideoClassifier:
    ...
    def extract_video_features(self, video_path):
        ...
        if len(frames) == 0:
            ...
            return None
        feats = self.intern_model.get_vid_feat(frames2tensor(frames, fnum=self.config.get('num_frames', 8), target_size=(self.config.get('size_t', 224), self.config.get('size_t', 224)), device='cuda'))
        return feats
    ...
    def extract_video_features_and_save(self, video_input_dir, feats_save_dir):
        ...
        for video_dir in video_files:
            ...
            video_folders = [x for x in os.listdir(video_dir_path) if os.path.isdir(os.path.join(video_dir_path, x))]
    ...
    def classify_videos(self, text_feats):
        ...
        for video_path in tqdm(video_files, desc="Processing videos"):
            ...
            if video_feat is not None:
                video_basename = os.path.basename(video_path)
                feats_save_path = os.path.join(self.feats_save_dir, f"{os.path.splitext(video_basename)[0]}.pth")
                torch.save({'feat': video_feat}, feats_save_path)
                video_paths_list.append({'feat': video_feat, 'path': video_path})

        # label_probs = (100.0 * torch.stack([v['feat'] for v in video_paths_list]) @ text_feats.T).softmax(dim=-1)
